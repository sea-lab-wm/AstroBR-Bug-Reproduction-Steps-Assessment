...
class LLM:
	def __init__(self):
		api_key = open("../ImportantInfo/openai_my.key").read()
	...
	def create_directory(self, cache_dir):
		...
		individual_s2rs_dir = cache_dir + "/temp_match_prompts"
		if not os.path.exists(individual_s2rs_dir):
			os.makedirs(individual_s2rs_dir)

		individual_s2rs_dir = cache_dir + "/temp_match_responses"
		if not os.path.exists(individual_s2rs_dir):
			os.makedirs(individual_s2rs_dir)

		comp_ids_prompts_dir = cache_dir + "/temp_comp_ids_prompts"
		if not os.path.exists(comp_ids_prompts_dir):
			os.makedirs(comp_ids_prompts_dir)
		
		comp_ids_responses_dir = cache_dir + "/temp_comp_ids_responses"
		if not os.path.exists(comp_ids_responses_dir):
			os.makedirs(comp_ids_responses_dir)
	...
	def get_response_for_temp_match(self, app_name, bug_id, s2r_index, s2r_sentence_list, comp_str, screen_id, activity):
		object_dir = self.cache_dir
		cur_filename = "Bug-" + str(bug_id) + "-S2R-" + str(s2r_index) +  "-Screen-" + str(screen_id) + ".txt"
		text_object_file = object_dir + "/temp_match_responses/" + cur_filename
		prompt_file = object_dir + "/temp_match_prompts/" + cur_filename
		...
		if self.check_if_score_computed(prompt_file) == True:
			...
			#match_prompt = self.prompts.create_few_shot_prompt_with_activity(app_name, s2r_sentence_list[s2r_index], comp_str, activity)
			#match_prompt = self.prompts.create_chain_of_thought_prompt_with_activity(app_name, s2r_sentence_list[s2r_index], comp_str, activity)
	...
	def get_response_for_temp_comp_ids(self, app_name, bug_id, s2r_index, s2r_sentence_list, comp_str, screen_id, activity):
		object_dir = self.cache_dir
		cur_filename = "Bug-" + str(bug_id) + "-S2R-" + str(s2r_index) +  "-Screen-" + str(screen_id) + ".txt"
		text_object_file = object_dir + "/temp_comp_ids_responses/" + cur_filename
		prompt_file = object_dir + "/temp_comp_ids_prompts/" + cur_filename
		text_exist = self.check_if_score_computed(text_object_file)
		id_prompt = ""
		response = ""
		...
		if self.check_if_score_computed(prompt_file) == True:
			...
			#id_prompt = self.prompts.identify_few_shot_interaction_id_with_activity(app_name, s2r_sentence_list[s2r_index], comp_str, activity)
			#id_prompt = self.prompts.identify_chain_of_thought_interaction_id_with_activity(app_name, s2r_sentence_list[s2r_index], comp_str, activity)
			if "\"" not in cur_filename and "/" not in cur_filename:
				with open(prompt_file, "w") as f:
					f.write(id_prompt)
					f.close()
