...
class WriteResults:
	...
	def write_individual_s2r_header(self, filename):
		with open(filename, 'w') as file:
			writer = csv.writer(file)
			writer.writerow(["Bug_id", "S2R Index", "Individual Step"])
	...
	def write_row_for_individual_s2rs(self, filename, bug_id, individual_s2r_list):
		for i in range(len(individual_s2r_list)):
			self.write_to_csv(filename, [bug_id, i+1, individual_s2r_list[i]])
